...
public class OrgFactoryTest extends RhnBaseTestCase {
    ...
    /**
     * Simple test illustrating how roles work. Note that the channel_admin role is implied
     * for an org admin iff the org has the channel_admin role.
     */
    ...
    public void testCreateOrg() throws Exception {
        ...
        assertEquals(new Long(1024L * 1024L * 1024L * 16L), org2.getOrgQuota().getTotal());
    }
    ...
    public void testOrgDefautRegistrationToken() throws Exception {
        ...
        // Token is hidden behind activation key so we have to look it up manually:
    ...
    /**
    * Test the addition of an entitlement to an org
    * This code should be refactored into a business
    * method of some sort if it becomes necessary
    * to actually add entitlements progmatically
    * from within the Java code.  For now we need
    * this test because new Orgs don't have any
    * entitlements.
    */
    ...
    public void testHasEntitlementFalse() throws Exception {
        Org org1 = OrgFactory.createOrg();
        OrgEntitlementType oet =
            OrgFactory.lookupEntitlementByLabel("sw_mgr_enterprise"); 
    ...
    */ 
    ...
    public void testAddServerGroup() throws Exception {
        ...
        for (Iterator itr = org1.getEntitledServerGroups().iterator(); itr.hasNext();) {
            ...
            assertNotNull(group);
            if (ServerConstants.getServerGroupTypeUpdateEntitled().
                                        equals(group.getGroupType())) {
                contains = true;
            }
        }
        assertTrue(contains);

        //in hosted, the hibernate mapping files were broken so that adding a second
        //server group would break a database constraint.  This line ensures that that
        //problem will not exist again.
        ServerGroupTest.createTestServerGroup(org1,
    ...
    /**
     * Test hibernate level-2 caching of OrgEntitlement Type
     * uncomment only if hibernate.show_sql=true
     */
    /*public void testCrap() throws Exception {
        //Org org1 = OrgFactory.lookupById(new Long(-1));
        OrgEntitlementType org1 = OrgFactory.lookupEntitlementByLabel("sw_mgr_personal");
        System.out.println(org1.getId());

        //Get one from the db
        OrgEntitlementType org2 = OrgFactory.lookupEntitlementByLabel("rhn_monitor");
        System.out.println(org2.getId());

        //Get it again... should come from cache
        OrgEntitlementType org3 = OrgFactory.lookupEntitlementByLabel("rhn_monitor");
        System.out.println(org3.getId());
 
        //Make sure is validEntitlement works...
        //OrgEntitlementType org4 = new OrgEntitlementType();
        //org4.setLabel("crap");
        //org4.setId(new Long(234334));
        //System.out.println(OrgFactory.isValidEntitlement(org4));
