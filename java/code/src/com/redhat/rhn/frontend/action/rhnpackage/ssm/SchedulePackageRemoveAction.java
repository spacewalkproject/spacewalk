...
public class SchedulePackageRemoveAction extends RhnListAction implements Listable {
    ...
    private ActionForward executePackageAction(ActionMapping mapping,
        ...
        // Find the servers in the SSM to add the events to
        RhnSet servers =
            RhnSetManager.findByLabel(user.getId(), RhnSetDecl.SYSTEMS.getLabel(), null);

        // An action must be created for each server in the SSM
        // that subscribes to the channel.
        for (RhnSetElement element : servers.getElements()) {
