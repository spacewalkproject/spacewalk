...
public class StringUtil {
    ...
    static final long[] MILLIS_PER_UNIT   = {1000, 60000, 3600000, 
                                                    86400000, 604800000,
                                                    2419200000L, 29030400000L};
    // Time-unit in next-higher unit (ie, 60 secs in 1 minute); used by interval-formatting.
    ...
    /**
     * Convert the passed in string to a valid java method name.  This
     * basically capitalizes each word and removes all word delimiters.
    ...
    /**
     * Converts the passed in string to a valid java Class name.  This
     * basically capitalizes each word and removes all word delimiters.
     * @param str The string to convert.
     * @return The converted string.
     */
    ...
    /** given a string, return the int of it; any parse error results
     * in 0 being returned 
    ...
    /** given a string, return the int of it; any parse error results
     * in default being returned 
     * @param s the String to convert
     * @param defaultValue the default value to assign to the return value
     * if the String didn't parse correctly
     * @return int the String converted to an int type
     */
    ...
    /** given a string and a map, all instances of {key} will be
     * replaced with the value of 'key' in the map.  so for example,
     * "this is a {noun}" with a Map that has ("noun" => "fish") would
     * produce "this is a fish"
    ...
    public static String replaceTags(String source, Map params) {
        ...
        while (i.hasNext()) {
            ...
            ret = StringUtils.replace(ret, "<" + me.getKey() + " />",
    ...
    /**
    * Using {@link java.util.StringTokenizer}, stringToList will
    * convert the string to a list of individual strings based on the 
    * standard default StringTokenizer behavior.  
    ...
    */
    ...
    /** 
      * Convert a class's name into the name without the package
      * For example StringUtils.getClassNameNoPackage(StringUtils.class) 
      * outputs:
      * StringUtils
    ...
    /**
     * Returns a String for html parsing
     * escapes html
     * converts \n to a break tag (&lt;BR/&lt;)
     * converts urls beginning with http:// and https:// to links
     * Example:
     * given http://foo.bar/example
     * return <a href="http://foo.bar/example">http://foo.bar/example</a>
    ...
    public static String htmlifyText(String convertIn) {
         ...
         * Break points for different tokens in the list are the
         * start of each hyperlink (http:// or https://).
        ...
        /*
         * Adds each piece of the list back to the string modifying each
         * that starts with http:// or https://
         * This part finds the end of each url and executes modifications
         */
     ...
     *
     * @param separator The String to glue the strings in inputList together with
     ...
     * Used by join(String,List) and acts as a convenience for the few times
     * we want to use join without a list.
    ...
    /**
     * Converts the number of bytes to the appropriate unit (B, KB, or MB) depending on
     * how many bytes there actually are.  It then localizes the display of this number
     * and formats the display with the units.  It will return fractional units for 
     * larger numbers (123.45 Mb and 123.4 Kb)
     ...
     * how many bytes there actually are.  It then localizes the display of this number
     * and formats the display with the units.  It will return fractional units for 
     * larger numbers (123.45 Mb and 123.4 Kb) unless wholeNum == true.
     * @param bytes the number of bytes used by a file
    ...
    public static String displayFileSize(long bytes, boolean wholeNum) {
        ...
        if (bytes >= (1024 * 1024)) { //show in megabytes (with two decimals)
            ...
            number = ls.formatNumber(new Double(bytes / 1024.0), 
                    (wholeNum ? 0 : 1));
            type = "kb";
        }
        ...
        /* the number that has now been localized is being used as an argument for
         * another localization call.
         * I'm not really sure if the format of displaying file sizes is 
         * truly something that needs to be localized. 
         */
    ...
    /**
     * Takes a target time (in msecs) and a maximum and minimum time-unit-of-interest and 
     * returns an I18N string in "x [maxUnits] y [units] ... z [minUnits] [ago:from now]" 
     * format.
     * 
     * @param target timestamp in msecs-since-epoch of the event
     * @param maxUnit constant representing the maximum unit you want to display.
     * @param minUnit constant representing the minimum unit you want to display.
     * @return I18N string in "x [maxUnits] y [units] ... z [minUnits] [ago:from now]
     * format
     * @throws IllegalArgumentException if maxUnit or minUnit not recognized,
     * or if maxUnit < minUnit
     */
    public static String categorizeTime(long target, int maxUnit, int minUnit) {
        ...
        for (int currUnit = maxUnit; currUnit >= minUnit; currUnit--) {
            if (currUnit == maxUnit) {
                ...
                elapsedTime -= 
                    (unitValues[currUnit + 1] * MILLIS_PER_UNIT[currUnit + 1]);
                unitValues[currUnit] =  elapsedTime / MILLIS_PER_UNIT[currUnit]; 
            }
        }
        ...
        LocalizationService ls = LocalizationService.getInstance();
        return ls.getMessage("timetag.categorizeLongFormat", 
                                    buff.toString().trim(), getTense(now, target));
    }
    ...
    /**
     * Takes a target time (in msecs) and a time-unit-of-interest and returns an I18N
     * string in "x {units} {ago:from now}" format.
     * 
     * @param target timestamp in msecs-since-epoch of the event
    ...
    public static String categorizeTime(long target, int maxUnit) {
        ...
        // maxUnit is now one of entered-value, largest-unit-smaller-than-elapsed,
        // or SECONDS_UNITS.  Figure out how-many units we have, and what the remainder is
        ...
        return ls.getMessage("timetag.categorizeFormat", 
                                valInMaxUnits, unitString, tense);
   }
     ...
     * @return Linux-EOL'd-string
     * TODO: This shoudl be generalized to handle other kinds of non-Linux
     * EOLs, so we can use it on uploaded files as well
     */
    ...
    /**
     * Basically turns an html or xml snippet to plain text
     * Meant to be used along with string resources xml file messages.
     * This is a best guess plain text conversion.. 
     * For example the following text
     * <pre><p>You donot have enough entitlements for <strong>
     *                   xyz system </strong></p>
    ...
    /**
     * Convert a string of options (name value pairs seperated by '=', where the pairs 
     *          are seperated by whitespace), into a map.
     * @param options the string of options
     * @param errorKey the localization key of the error message to throw if we can't parse 
     *              it correctly
     * @return  a map containing name value pairs of options
     * @throws ValidatorException if there isn't an '=' sign seperating the pairs
     */
    ...
    /**
     * Convert a map of kernel options into a string:
