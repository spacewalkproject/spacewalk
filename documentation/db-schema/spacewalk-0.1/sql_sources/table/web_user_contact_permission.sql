-- created by Oraschemadoc Fri Jun 13 14:05:58 2008
-- visit http://www.yarpen.cz/oraschemadoc/ for more info

  CREATE TABLE "RHNSAT"."WEB_USER_CONTACT_PERMISSION" 
   (	"WEB_USER_ID" NUMBER CONSTRAINT "WUCP_WUID_NN" NOT NULL ENABLE, 
	"EMAIL" CHAR(1) DEFAULT ('N'), 
	"MAIL" CHAR(1) DEFAULT ('N'), 
	"CALL" CHAR(1) DEFAULT ('N'), 
	"FAX" CHAR(1) DEFAULT ('N'), 
	"CREATED" DATE DEFAULT (sysdate) CONSTRAINT "WUCP_CREATED_NN" NOT NULL ENABLE, 
	"MODIFIED" DATE DEFAULT (sysdate) CONSTRAINT "WUCP_MODIFIED_NN" NOT NULL ENABLE, 
	 CONSTRAINT "WUCP_EMAIL_CK" CHECK (email in ('Y','N')) ENABLE, 
	 CONSTRAINT "WUCP_MAIL_CK" CHECK (mail in ('Y','N')) ENABLE, 
	 CONSTRAINT "WUCP_CALL_CK" CHECK (call in ('Y','N')) ENABLE, 
	 CONSTRAINT "WUCP_FAX_CK" CHECK (fax in ('Y','N')) ENABLE, 
	 CONSTRAINT "CONTPERM_WBUSERID_FK" FOREIGN KEY ("WEB_USER_ID")
	  REFERENCES "RHNSAT"."WEB_CONTACT" ("ID") ON DELETE CASCADE ENABLE
   ) PCTFREE 10 PCTUSED 40 INITRANS 1 MAXTRANS 255 NOCOMPRESS LOGGING
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
  TABLESPACE "DATA_TBS" 
 
/
