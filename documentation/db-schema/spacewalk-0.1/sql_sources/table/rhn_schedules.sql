-- created by Oraschemadoc Fri Jun 13 14:05:58 2008
-- visit http://www.yarpen.cz/oraschemadoc/ for more info

  CREATE TABLE "RHNSAT"."RHN_SCHEDULES" 
   (	"RECID" NUMBER(12,0) CONSTRAINT "RHN_SCHED_RECID_NN" NOT NULL ENABLE, 
	"SCHEDULE_TYPE_ID" NUMBER(12,0) CONSTRAINT "RHN_SCHED_TYPE_ID_NN" NOT NULL ENABLE, 
	"DESCRIPTION" VARCHAR2(40) DEFAULT 'unknown' CONSTRAINT "RHN_SCHED_DESC_NN" NOT NULL ENABLE, 
	"LAST_UPDATE_USER" VARCHAR2(40), 
	"LAST_UPDATE_DATE" DATE, 
	"CUSTOMER_ID" NUMBER(12,0), 
	 CONSTRAINT "RHN_SCHED_RECID_CK" CHECK (recid > 0) ENABLE, 
	 CONSTRAINT "RHN_SCHED_RECID_PK" PRIMARY KEY ("RECID")
  USING INDEX PCTFREE 10 INITRANS 32 MAXTRANS 255 COMPUTE STATISTICS 
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 16 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
  TABLESPACE "DATA_TBS"  ENABLE, 
	 CONSTRAINT "RHN_SCHED_CSTMR_CUST_ID_FK" FOREIGN KEY ("CUSTOMER_ID")
	  REFERENCES "RHNSAT"."WEB_CUSTOMER" ("ID") ENABLE, 
	 CONSTRAINT "RHN_SCHED_SCHTP_SCHED_TY_FK" FOREIGN KEY ("SCHEDULE_TYPE_ID")
	  REFERENCES "RHNSAT"."RHN_SCHEDULE_TYPES" ("RECID") ENABLE
   ) PCTFREE 10 PCTUSED 40 INITRANS 32 MAXTRANS 255 NOCOMPRESS LOGGING
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 16 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
  TABLESPACE "DATA_TBS" 
 
/
