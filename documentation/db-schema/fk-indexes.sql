--missed index on QRTZ_TRIGGERS table for SYS_C006582 constraint
...
create index fk_no_index_191 on RHN_HOST_PROBE (PROBE_ID, PROBE_TYPE);
...
create index fk_no_index_197 on RHN_SAT_CLUSTER_PROBE (PROBE_ID, PROBE_TYPE);

--missed index on RHN_SAT_NODE table for RHN_SATND_CMDTG_RID_TAR_TY_FK constraint 
create index fk_no_index_198 on RHN_SAT_NODE (RECID, TARGET_TYPE);

--missed index on RHN_SAT_NODE_PROBE table for RHN_SNDPB_PR_RECID_PR_TYP_FK constraint 
create index fk_no_index_199 on RHN_SAT_NODE_PROBE (PROBE_ID, PROBE_TYPE);
...
create index fk_no_index_201 on RHN_URL_PROBE (PROBE_ID, PROBE_TYPE);

--missed index on VALID_COUNTRIES_TL table for VALID_COUNTRIES_TL_CODE constraint 
create index fk_no_index_202 on VALID_COUNTRIES_TL (CODE);

--missed index on WEB_CUSTOMER_NOTIFICATION table for WEB_CUST_NOT_OID_FK constraint 
create index fk_no_index_203 on WEB_CUSTOMER_NOTIFICATION (ORG_ID);
