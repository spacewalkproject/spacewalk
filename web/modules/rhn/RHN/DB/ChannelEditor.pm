...
#checks that org has a channelfamily associated with it.  If not, creates one.
sub verify_org_family {

  my $class = shift;
  my $org = shift or return;
...
EOQ
  ...
  unless (@cf_ids) { #make a channelfamily for this org

    $query =<<EOQ;
INSERT INTO  rhnChannelFamily
             (id, name, label, org_id, product_url)
     VALUES  (rhn_channel_family_id_seq.nextval, ?, ?, ?, ?)
EOQ

    $sth = $dbh->prepare($query);
    $sth->execute($org->name . " (" . $org->id . ") " . "Channel Family", "private-channel-family-".$org->id, $org->id, "org_channel_family.pxt");
...
#checks that org has a channelfamilypermission for it's channelfamily
#grants them if it does not.
#Takes an org object, and returns the rhnChannelFamily.id(s) added, if any.
...
EOQ
  ...
  foreach my $cf_id (@cf_ids) {
    $sth->execute($cf_id, $org->id);
  }
