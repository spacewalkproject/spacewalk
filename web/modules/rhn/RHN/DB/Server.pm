...
sub activate_satellite {
  my $self = shift;
  my $cert_str = shift;
  my $transaction = shift;

  if ($self->is_proxy()) {
    RHN::API::Exception->throw_named("satellite_system_is_proxy");
  }

  die "no cert string!" unless ($cert_str);

  #  test cert for validity...
  ...
  if (not $cert->get_field('generation')) {
    warn "attempt to use satellite cert without 'generation' field";
    RHN::API::Exception->throw_named('satellite_cert_too_old');
  }

  my $org = RHN::Org->lookup(-id => $self->org_id);

  my $base_channel_id = $self->base_channel_id;
  if (not $base_channel_id) {
    RHN::API::Exception->throw_named("satellite_no_base_channel");
  }

  my $sat_chan_info;

  eval {
    $sat_chan_info = $self->sat_channel_by_version(-version => $cert->version() || '1.0');
  };
  if ($@) {
    my $E = $@;
    if ($E =~ /invalid satellite version/) {
      RHN::API::Exception->throw_named("no_sat_chan_for_version");
    }

    throw $E;
  }

  RHN::API::Exception->throw_named("no_sat_chan_for_version") unless $sat_chan_info;

  RHN::API::Exception->throw_named("no_access_to_sat_channel") unless $org->has_channel_permission($sat_chan_info->{CHANNEL_ID});
  ...
  # if it's already a satellite (i.e., upgrade), reuse logic for blowing away old stuff...
  $dbh = $self->deactivate_satellite($dbh);

  $query = <<EOQ;
INSERT INTO  rhnSatelliteInfo (server_id, cert, product, owner, issued_string, expiration_string, evr_id)
     VALUES  (:sid, :cert, :product, :owner, :issued_string, :expiration_string, lookup_evr(NULL, :version, '1'))
EOQ

  $sth = $dbh->prepare($query);

  $sth->execute_h(sid => $self->id,
		  cert => $dbh->encode_blob($cert_str, 'cert'),
		  product => $cert->get_field('product'),
		  owner => $cert->get_field('owner'),
		  issued_string => $cert->get_field('issued'),
		  expiration_string => $cert->get_field('expires'),
		  version => $cert->get_field('satellite-version'));
...
EOQ
  ...
  my $management_slots = $cert->get_field('slots');

  if ($cert->get_field('nonlinux-slots')) {
    $management_slots += $cert->get_field('nonlinux-slots');
  }

  $sth->execute_h(sid => $self->id, group_type => 'enterprise_entitled', max_members => $management_slots);
  $sth->execute_h(sid => $self->id, group_type => 'provisioning_entitled', max_members => $cert->get_field('provisioning-slots'))
    if $cert->get_field('provisioning-slots');
  $sth->execute_h(sid => $self->id, group_type => 'monitoring_entitled', max_members => $cert->get_field('monitoring-slots'))
    if $cert->get_field('monitoring-slots');
...
EOQ
  ...
  for my $cfam_entitlement ($cert->get_channel_families) {
    $sth->execute_h(sid => $self->id,
		    channel_family_label => $cfam_entitlement->[0],
		    quantity => $cfam_entitlement->[1]);
  }

  $self->subscribe_to_channel($sat_chan_info->{CHANNEL_LABEL}, $dbh);
