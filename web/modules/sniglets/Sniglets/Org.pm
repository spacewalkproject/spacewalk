...
sub register_callbacks {
  ...
  $pxt->register_callback('rhn:create_cert_cb' => \&create_cert_cb);
...
sub create_cert_cb {
  ...
  my $action = $pxt->dirty_param('create_cert');
  ...
  if ($action eq 'Back') {
    ...
    $pxt->redirect("/internal/support/org_entitlements.pxt?support_org_id=$support_org_id");
    return;
  }
  ...
  if ($action eq 'Reset') {
    ...
    # update any meta info
    $cert_info{'meta_info'}->{'version'} = $pxt->dirty_param('version');

    if ( $pxt->dirty_param('owner') ) {
      $cert_info{'meta_info'}->{'owner'} = $pxt->dirty_param('owner');
    }
    else {
      $pxt->push_message(local_alert => "An owner must be specified.");
      $error = 1;
    }
    ...
    my $issue_secs;
    my $expires_secs;
    if (str2time($pxt->dirty_param('issued'))) {
      $issue_secs = str2time($pxt->dirty_param('issued'),'GMT');
      $cert_info{'meta_info'}->{'issued'} =  HTTP::Date::time2str($issue_secs);
    }
    else {
      $pxt->push_message(local_alert => "Invalid issued date.");
      $error = 1;
    }
    if (str2time($pxt->dirty_param('expires'))) {
      $expires_secs = str2time($pxt->dirty_param('expires'),'GMT');
      if ($expires_secs  > $issue_secs) {
        $cert_info{'meta_info'}->{'expires'} = 
          HTTP::Date::time2str($expires_secs);
      }
      else {
        $pxt->push_message(local_alert => "Expires date must be greater than issued date.");
        $error = 1;
      }
    }
    else {
      $pxt->push_message(local_alert => "Invalid expires date.");
      $error = 1;
    }

    # services
    my @services = $pxt->dirty_param('service');
    my $service_set = RHN::Set->lookup(-label => 'new_cert_service_set', -uid => $pxt->user->id);
    my $service_label_to_id = RHN::Server->server_group_type_ids_by_label;
    my %service_id_to_label = reverse %{$service_label_to_id};

    foreach my $id (@services) {
      my $quant = $pxt->dirty_param("service_". $id . "_quant");
      my $label = $service_id_to_label{$id};

      if ($quant =~ /\D/) {
	$pxt->push_message(local_alert => "Invalid quantity entered for $label service.");
	$error = 1;
      }
      elsif ($label eq 'enterprise_entitled' and ($quant + 0) eq 0) {
	$pxt->push_message(local_alert => "Must have at least 1 enterprise entitlement slot.");
	$error = 1;
      }
      else {
	my $old_quant = $pxt->dirty_param("service_" . $id . "_oldquant");
	$service_set->remove( [$id, $old_quant + 0] );
	$service_set->add( [$id, $quant + 0] );
      }
    }
    ...
    # channels  
    my @channels = $pxt->dirty_param('chanid');
    my $channel_set = RHN::Set->lookup(-label => 'new_cert_channel_set', -uid => $pxt->user->id);
    foreach my $id (@channels) {
        my $quant = $pxt->dirty_param("chanid_" . $id . "_quant");
        my $label = $pxt->dirty_param("chanid_" . $id . "_label");
        if ($quant =~ /\D/) {

          $pxt->push_message(local_alert => "Invalid quantity entered for channel $label.");
          $error = 1;
        }
        else {
          my $old_quant = $pxt->dirty_param("chanid_" . $id . "_oldquant");
          $channel_set->remove( [$id, $old_quant + 0] );
          $channel_set->add( [$id, $quant + 0] );
        }
    }

    # add'l channels
    my @addchannels = $pxt->dirty_param('addchanid');
    my $addchannel_set = RHN::Set->lookup(-label => 'new_cert_add_channel_set', -uid => $pxt->user->id);
    foreach my $id (@addchannels) {
      my $quant = $pxt->dirty_param("addchanid_" . $id . "_quant");
      if ($quant =~ /\D/) {
          my $label = $pxt->dirty_param("addchanid_" . $id . "_label");
          $pxt->push_message(local_alert => "Invalid quantity entered for additional channel $label.");
          $error = 1;
      }
      else {
        my $old_quant = $pxt->dirty_param("addchanid_" . $id . "_oldquant");
        $addchannel_set->remove( [$id, $old_quant + 0] );
        if ($quant > 0) {
           $addchannel_set->add( [$id, $quant + 0] );
        }
      }
    }

    # adjust rhn slots in case provisioning entitlements changed.
    my %service_hash = $service_set->output_hash;
    my $prov_quant = $service_hash{$service_label_to_id->{'provisioning_entitled'}};
    my $rhn_tools_id = RHN::Channel->family_details_by_label('rhn-tools')->{ID};
    my $rhn_tools_old_quant = $pxt->dirty_param("addchanid_" . $rhn_tools_id . "_oldquant");
    my %contents = $channel_set->output_hash;
    if ( $contents{$rhn_tools_id} ) {
       $channel_set->remove( [$rhn_tools_id , $rhn_tools_old_quant + 0] );
       $channel_set->add( [$rhn_tools_id , $prov_quant + 0] );
    }
    else {
       $addchannel_set->remove( [$rhn_tools_id , $rhn_tools_old_quant + 0] );
       $addchannel_set->add( [$rhn_tools_id , $prov_quant + 0] );
    }


    # add in a newly defined channel, if any
    if ( $pxt->dirty_param('newchannel') ) {
       my $id = $pxt->dirty_param('newchannel');
       my $quant = $pxt->dirty_param('newchannel_quant');
       if ($quant =~ /\D/) {
         my $chanfam = RHN::Channel->family_details($id);
         my $add_channel_label =  $chanfam->{LABEL};
         $add_channel_id =  $id;
         $pxt->push_message(local_alert => sprintf("Invalid quantity entered for additional channel %s.", $add_channel_label ));
         $error = 1;
       }
       else {
         if ($quant > 0) {
    ...
    $channel_set->commit();
    $addchannel_set->commit();
  }

  my $redir = $pxt->uri . "?support_org_id=" . $support_org_id;

  if (not $error and $action eq "Download Cert") {
    download_cert($pxt, \%cert_info);
    return;
  }
  elsif ($action eq "View Cert") {
    $redir = ("cert_text.pxt?support_org_id=" . $support_org_id) unless ($error);
  }
  elsif ($action eq "Add Channel") {
    # add a channel
    $add_channel = 1; 
    $redir .= $add_channel ? '&add_channel=1' : '';
    if ($error) {
      $redir .= '&add_channel_id=' . $add_channel_id;
    }
  }
