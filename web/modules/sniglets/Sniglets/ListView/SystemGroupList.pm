...
sub _register_modes {
  ...
  Sniglets::ListView::List->add_mode(-mode => "groups_a_system_is_in",
			   -datasource => RHN::DataSource::SystemGroup->new,
                           -action_callback => \&remove_system_from_groups_cb);

  Sniglets::ListView::List->add_mode(-mode => "groups_a_system_is_in_admin",
                           -datasource => RHN::DataSource::SystemGroup->new,
                           -action_callback => \&remove_system_from_groups_cb);

  Sniglets::ListView::List->add_mode(-mode => "groups_a_system_is_not_in",
			   -datasource => RHN::DataSource::SystemGroup->new,
                           -action_callback => \&add_system_to_groups_cb);

  Sniglets::ListView::List->add_mode(-mode => "groups_a_system_is_not_in_admin",
                           -datasource => RHN::DataSource::SystemGroup->new,
                           -action_callback => \&add_system_to_groups_cb);
...
sub add_system_to_groups_cb {
  my $self = shift;
  my $pxt = shift;
  my %action = @_;

  if (exists $action{label} and $action{label} eq 'add_system_to_groups') {
    my $sid = $pxt->param('sid');
    throw "no system id" unless $sid;

    my $set_label = 'target_groups_for_system';
    my $set = RHN::Set->lookup(-label => $set_label, -uid => $pxt->user->id);
    my $system = RHN::Server->lookup(-id => $sid);

    my @systems = ($system->id);
    my @groups = $set->contents;

    unless (@groups) {
      $pxt->push_message(local_alert => "No groups selected.");
      return 1;
    }

    throw sprintf('user (%d) does not have access to one or more system groups: (%s)', $pxt->user->id, join(', ', @systems))
	unless $pxt->user->verify_system_group_access(@groups);

    my $transaction = RHN::DB->connect();
    $transaction->nest_transactions();

    eval {
      RHN::Server->add_servers_to_groups(\@systems, \@groups, $transaction);
      if ($system->has_feature('ftr_snapshotting')) {
        RHN::Server->snapshot_server(-server_id => $system->id,
                                     -reason => "Group membership alteration",
    ...
    $pxt->push_message(site_info => sprintf("<strong>%s</strong> added to <strong>%d</strong> group%s.",
                                            PXT::Utils->escapeHTML($system->name), scalar(@groups), (scalar(@groups) == 1 ? '' : 's')));
    $set->empty;
    $set->commit;
  }
  return 1;
...
sub remove_system_from_groups_cb {
  my $self = shift;
  my $pxt = shift;
  my %action = @_;

  if (exists $action{label} and $action{label} eq 'leave_system_groups') {
    my $sid = $pxt->param('sid');
    throw "no system id" unless $sid;

    my $set_label = 'remove_system_from_groups';
    my $set = RHN::Set->lookup(-label => $set_label, -uid => $pxt->user->id);
    my $system = RHN::Server->lookup(-id => $sid);
    ...
    eval {
      RHN::Server->remove_servers_from_groups(\@systems, \@groups, $transaction);
      if ($system->has_feature('ftr_snapshotting')) {
        RHN::Server->snapshot_server(-server_id => $system->id,
                                     -reason => "Group membership alteration",
                                     -transaction => $transaction);
    ...
    $pxt->push_message(site_info => sprintf("<strong>%s</strong> removed from <strong>%d</strong> group%s.",
                                            PXT::Utils->escapeHTML($system->name), scalar(@groups), (scalar(@groups) == 1 ? '' : 's')));

    $set->empty;
    $set->commit;

    if (not $pxt->user->verify_system_access($sid)) {
       $pxt->redirect('/rhn/systems/Overview.do');
    }
  }
