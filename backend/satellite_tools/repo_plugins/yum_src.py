...
from custom_update_md import UpdateMetadata,UpdateNotice,UpdateNoticeException
...
class ContentSource(object):
    ...
    def setup_repo(self, repo, no_mirrors, ca_cert_file, client_cert_file, client_key_file):
        ...
        except:              # dnf bug workaround. Mirrorlist was provided but none worked. Fallback to baseurl and load again. Remove once dnf is fixed and add detection if mirrors failed
            repo.mirrorlist = ""
            no_mirrors = True
            self.dnfbase.repos[self.repoid].load()
        ...
        if no_mirrors:
            ...
            self.dnfbase.repos[self.repoid].urls = self.clean_urls(self.dnfbase.repos[self.repoid]._repo.getMirrors())
    ...
    def _filter_packages(self, packages, filters):
        ...
        for filter_item in filters:
            ...
            if sense == '+':
                ...
                raise MarkingErrors("Invalid filter sense: '%s'" % sense)
        return selected

    def __findDeps(self, pkgSack, pkgs):
#
...
#
    ...
    def set_download_parameters(self, params, relative_path, target_file, checksum_type=None, checksum_value=None,
        ...
        params['proxies'] = get_proxies(self.dnfbase.repos[self.repoid].proxy, self.dnfbase.repos[self.repoid].proxy_username, self.dnfbase.repos[self.repoid].proxy_password)
    ...
    def get_metadata_paths(self):
        ...
        repomd_path = os.path.join(self.dnfbase.repos[self.repoid].basecachedir, self.name + "-" + self.digest, "repodata", "repomd.xml")
