...
def load(filename=None, file=None, fd=None, seekable=1):
    ...
    p = MPM_Package()
    try:
        p.load(f, seekable=seekable)
    except InvalidPackageError, e:
        try:
            return load_rpm(f, seekable=seekable)
        except InvalidPackageError:
            raise e

    return p.header, p.payload_stream

def load_rpm(stream, seekable=1):
    # Hmm, maybe an rpm
...
class MPM_Package:
    ...
    def load(self, input_stream, seekable=0):
        # Clean up
        self.__init__()
