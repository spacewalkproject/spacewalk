...
class ChannelsDumper(BaseDumper):
    ...
    def dump_subelement(self, data):
        c = ChannelDumper(self._writer, data)
        c.dump()


class ChannelDumper(BaseRowDumper):
    ...
    def _get_package_ids(self):
        ...
        h.execute(channel_id=channel_id)
        return [x['package_id'] for x in h.fetchall_dict() or []]
    ...
    def _get_errata_ids(self):
        ...
        h = rhnSQL.prepare(self._query__get_errata_ids)
        h.execute(channel_id=channel_id)
        return [x['errata_id'] for x in h.fetchall_dict() or []]
...
class _PackageDumper(BaseRowDumper):
    ...
    def set_attributes(self):
        attrs = ["name", "version", "release", "package_arch",
            ...
            "build_host", "source_rpm", "checksum", "payload_format",
    ...
    def set_iterator(self):
        ...
        h = rhnSQL.prepare("""
            ...
            from rhnPackageFile pf, rhnPackageCapability pc, rhnChecksum c
...
class ShortPackagesDumper(BaseDumper):
    ...
    def dump_subelement(self, data):
        ...
        attrs = [
            ...
            "package_arch", "checksum", "package_size", "org_id",
        ]
        for attr in attrs:
...
class SourcePackagesDumper(BaseDumper):
    ...
    def dump_subelement(self, data):
        ...
        attrs = [
            ...
            "payload_size", "build_host", "sig_checksum", "vendor",
            "cookie", "package_size", "checksum"
        ]
        for attr in attrs:
            attributes[attr.replace('_', '-')] = data[attr]
...
class _ChangelogEntryDumper(BaseRowDumper):
    ...
    def set_iterator(self):
        ...
        arr.append(SimpleDumper(self._writer,
...
class _ErratumDumper(BaseRowDumper):
    ...
    def set_iterator(self):
        ...
        _query_errata_file_info = """
             select ef.id errata_file_id, c.checksum,
...
class _ErratumBugDumper(BaseRowDumper):
    ...
    def set_iterator(self):
        arr = [
            SimpleDumper(self._writer, 'rhn-erratum-bug-id',
                self._row['bug_id']),
...
class _ErratumFilesDumper(BaseDumper):
    ...
    def dump_subelement(self, data):
        ...
        if data['type'] == 'RPM':
            ...
            if package_id is not None:
                attributes['source-package'] = 'rhn-package-source-%s' \
                        % package_id
...
class _KickstartableTreeDumper(BaseRowDumper):
    ...
    def set_attributes(self):
        attrdict = self._row.copy()
        del attrdict['id']
        last_modified = attrdict['last-modified']
        attrdict['last-modified'] = _dbtime2timestamp(last_modified)
        return attrdict
