...
# $Id: translate.py,v 1.3 2002/09/05 22:30:09 taw Exp $
...
def _intToStr(int):
    if _gettext_byteorder == 'msb':
        return _intToMsbStr(int)
    else:
        return _intToLsbStr(int)
...
class Catalog:
    def __init__(self, domain=None, localedir=LOCALE_DIR):
        self.domain = domain
...
def bindtextdomain(domain, localedir=LOCALE_DIR):
    global _cat
...
def test():
    ...
    if len(sys.argv) not in (2, 3):
        print "Usage: %s DOMAIN [LOCALE_DIR]" % (sys.argv[0],)
        sys.exit(1)
    domain = sys.argv[1]
    if len(sys.argv) == 3:
