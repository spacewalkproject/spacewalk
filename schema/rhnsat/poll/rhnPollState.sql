...
(
        id              number
			constraint rhn_poll_state_id_nn not null
                        constraint rhn_poll_state_id_pk primary key
				using index tablespace rhn_ind
				storage ( initial 40960 
					next 507904 pctincrease 1 ),
	text		varchar2(32)
			constraint rhn_poll_state_name_nn not null
)
storage ( initial 40960 next 5816320 pctincrease 1 );

insert into rhnPollState values (1,'NEW');
insert into rhnPollState values (2,'PUBLISHED');
