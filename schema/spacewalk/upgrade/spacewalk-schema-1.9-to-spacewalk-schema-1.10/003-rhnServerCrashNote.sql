...
(
    ...
    creator    NUMBER
                   CONSTRAINT rhn_server_crash_note_creator_fk
                       REFERENCES web_contact (id)
                       ON DELETE SET NULL,
    crash_id  NUMBER NOT NULL
                   CONSTRAINT rhn_crashrnotes_sid_fk
...
CREATE INDEX rhn_server_crash_note_sid_idx
    ON rhnServerCrashNote (crash_id)
    TABLESPACE [[64k_tbs]]
    NOLOGGING;

CREATE INDEX rhn_server_crash_note_creator_idx
    ON rhnServerCrashNote (creator)
    TABLESPACE [[64k_tbs]]
    NOLOGGING;

CREATE SEQUENCE rhn_server_crash_note_id_seq;
